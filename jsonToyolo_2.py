...
def convert_infolks_json(name, train_files, val_files, train_img_path, val_img_path):
    """将INFOLKS JSON注释转换为YOLO格式标签"""
    path = make_dirs()

    # 导入json
    train_data = []
    for file in glob.glob(train_files):
        with open(file) as f:
            jdata = json.load(f)
            jdata["json_file"] = file
            train_data.append(jdata)

    val_data = []
    for file in glob.glob(val_files):
        with open(file) as f:
            jdata = json.load(f)
            jdata["json_file"] = file
            val_data.append(jdata)

    # 写入图片和形状
    name = path + os.sep + name
    file_name, wh, cat = [], [], []
    for x in tqdm(train_data, desc="处理训练集文件和形状"):
        json_stem = Path(x["json_file"]).stem
        img_file = glob.glob(train_img_path + json_stem + ".png")

        if not img_file:
            print(f"找不到与 {json_stem} 对应的训练集图片文件")
            continue
        ...
        # 文件名
        with open(name + "_train.txt", "a") as file:
            file.write("%s\n" % f)

    for x in tqdm(val_data, desc="处理验证集文件和形状"):
        json_stem = Path(x["json_file"]).stem
        img_file = glob.glob(val_img_path + json_stem + ".png")

        if not img_file:
            print(f"找不到与 {json_stem} 对应的验证集图片文件")
            continue
        ...
        # 文件名
        with open(name + "_val.txt", "a") as file:
            file.write("%s\n" % f)
    ...
    # 写入标签文件
    for i, x in enumerate(tqdm(train_data, desc="处理训练集注释")):
        if i >= len(file_name):
            continue
    ...
    for i, x in enumerate(tqdm(val_data, desc="处理验证集注释")):
        if i >= len(file_name):
            continue
...
if __name__ == "__main__":
    convert_infolks_json(
        name="casia",
        train_files="/root/yolov8/ultralytics/datasets/CASIA/train/mask/*.json",
        val_files="/root/yolov8/ultralytics/datasets/CASIA/val/mask/*.json",
        train_img_path="/root/yolov8/ultralytics/datasets/CASIA/train/image/",
        val_img_path="/root/yolov8/ultralytics/datasets/CASIA/val/image/"
    )
